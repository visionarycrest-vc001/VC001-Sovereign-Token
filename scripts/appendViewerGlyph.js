...
if (glyph.length > 5) {
  console.error(
    "❌ ERROR: Glyph should be 5 characters or less for table formatting"
  );
  process.exit(1);
}
if (name.length > 22) {
  console.error(
    "❌ ERROR: Name should be 22 characters or less for table formatting"
  );
  process.exit(1);
}
if (purpose.length > 34) {
  console.error(
    "❌ ERROR: Purpose should be 34 characters or less for table formatting"
  );
  process.exit(1);
}
if (location.length > 28) {
  console.error(
    "❌ ERROR: Location should be 28 characters or less for table formatting"
  );
  process.exit(1);
}
...
async function appendViewerGlyph() {
  try {
    ...
    if (!fs.existsSync(registryPath)) {
      ...
      console.error(
        "   Please ensure you are running this script from the VC001 repository"
      );
      console.error(
        "   and that VC_ViewerRegistry.md exists in the repository root."
      );
      process.exit(1);
    }
    ...
    if (!content.includes("| Glyph |") || !content.includes("|-------|")) {
      console.error(
        "❌ ERROR: Registry file does not contain expected markdown table structure"
      );
      console.error(
        "   Expected headers: | Glyph | Name | Purpose | Activated In |"
      );
    ...
    if (tableRows.length < 2) {
      ...
      console.error(
        "   The registry must contain at least a header row and separator."
      );
      process.exit(1);
    }
    ...
    if (duplicate) {
      console.log(
        "🛑 DUPLICATE DETECTED: Glyph or name already exists in registry"
      );
